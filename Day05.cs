...
public class Day05 : DayX
{
    ...
    private int GetBoardSize(List<Line> lines)
    {
        int size = int.MinValue;

        foreach (var line in lines)
        {
            size = Math.Max(size, Math.Max(line.Start.X, line.End.X));
            size = Math.Max(size, Math.Max(line.Start.Y, line.End.Y));
        }
    ...
    private List<Line> GetLines(string input)
    {
        ...
        return input
            ...
            .Select(x => 
            {
                var pointPairs = x.Split(" -> ");

                var start = valuesSplitter(pointPairs[0]);
                var end = valuesSplitter(pointPairs[1]);
...
5,5 -> 8,2";
    ...
    const string input = @"529,822 -> 529,562
106,230 -> 568,230
709,377 -> 708,376
600,544 -> 600,638
75,351 -> 70,351
